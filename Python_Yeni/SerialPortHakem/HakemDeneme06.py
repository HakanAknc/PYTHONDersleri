...
class SerialReceiverWidget(QWidget):
    def __init__(self):
        ...
        # Veri alma zamanlayıcısını başlat
        self.receive_timer = QTimer()
        self.receive_timer.timeout.connect(self.receive_data)
        self.receive_timer.start(100) # her 100ms'de bir receive_data fonksiyonunu çağır
    ...
    def receive_data(self):
        try:
            ...
            selected_port = self.port_list_widget.currentText()
            baudrate = int(self.baudrate_widget.currentText())
            with serial.Serial(selected_port, baudrate=baudrate, timeout=0.1) as serial_port:
                # Veri al
                data = serial_port.read_all().decode()
                if data:
                    self.receive_list_widget.addItem(data)
        except serial.SerialException as e:
            QMessageBox.critical(self, "Hata", f"Port açılırken bir hata oluştu: {e}")
        except ValueError as e:
            QMessageBox.critical(self, "Hata", f"Baudrate geçersiz: {e}")
